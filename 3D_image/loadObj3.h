...
// light description
typedef struct {
	bool isEnabled;	// is this light active
	bool isLocal;	// true for point light source, false for directional light
	bool isSpot;	// true for spotlight
...
typedef enum { BLINN_PHONG, COOK_TORRANCE, GOOCH } shaderType;
...
typedef struct {
	...
	GLuint diffuseTexMapID;	// texture unit ID
	char bumpMap[128];		// file name of bump map
	GLuint bumpMapTexID;		// texture unit ID
} objInfo;
...
// memory for theObjects array is allocated and managed within loadObjFile

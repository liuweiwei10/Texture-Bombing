...
#include "loadObj3.h"
void loadTexture(char*, objInfo*, GLuint);
...
int loadObjFile(char *fileName, objInfo **theObjects, int *numObjects)
{
	...
	fileBounds[1] = fileBounds[3] = fileBounds[5] = -FLT_MAX;
	...
	{
		if((strcmp(objList[i].diffuseTex, "") != 0))
		{
           //cout << "texture map " << i  << ": " << objList[i].diffuseTex <<  "\n";  
           int width, height, channels;
           unsigned char* image;
           char filePath[128];
		   strcpy(filePath, pathToFile);
		   strcat(filePath, objList[i].diffuseTex);
           loadTexture(filePath, &objList[i], i);
		}
	}**/
...
void loadTexture(char* textureFileName, objInfo* obj, GLuint i)
{   
	unsigned char* image;
	int width; 
	int height; 
    ...
    //glGenTextures(1, &(obj.diffuseTexMapID));
    //glBindTexture(GL_TEXTURE_2D, obj->diffuseTexMapID);
